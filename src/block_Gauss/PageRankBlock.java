...
public class PageRankBlock {
	...
	public static void main(String[] args) throws Exception {
        ...
        do {
            ...
            float avgIterPerBlock = (float)job.getCounters().findCounter(MRCounters.TOTAL_INNER_ITERATION).getValue() / (float)totalBlocks;
            System.out.println("\nResidual error for iteration " + i + ": " + residualErrorString);
            System.out.println("Average number of iterations per block: " + avgIterPerBlock);
            System.out.println("\n=================== Iteration " + i + " END ===================\n" );
            
            
            // reset Hadoop counter
            job.getCounters().findCounter(MRCounters.RESIDUAL_ERROR).setValue(0L);
            job.getCounters().findCounter(MRCounters.TOTAL_INNER_ITERATION).setValue(0L);
            i++;
